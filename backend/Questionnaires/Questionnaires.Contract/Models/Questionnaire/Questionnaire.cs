...
public class QuestionnaireDelta
{
    ...
    public static QuestionnaireDelta Merge(QuestionnaireId id, params QuestionnaireDelta[] deltas)
    {
        if (deltas.Any(d => d.Id != id))
            throw new InvalidOperationException("All deltas must have the same Id");

        var merged = new QuestionnaireDelta { Id = id };
